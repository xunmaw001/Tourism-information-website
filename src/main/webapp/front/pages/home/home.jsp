...
<body>
  <div id="app">
    ...
    <div class="part2" style="margin-top: 50px;">
      <div class="part2_1">
        ...
        <div class="more">
            <a href="#" @click="jump('../lvyoujingdian/list.jsp')">MORE>></a>
        </div>
      </div>
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in lvyoujingdianRecommend" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.fengmian?item.fengmian.split(',')[0]:''" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../lvyoujingdian/detail.jsp?id='+item.id">查看详情</a>
            </div>
          </div>
          <div class="caseBox_2" style="text-align: center;">
            <a :href="'../lvyoujingdian/detail.jsp?id='+item.id">{{item.jingdian}}</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="part2" style="margin-top: 50px;">
      ...
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in lvyoujingdianList" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.fengmian?item.fengmian.split(',')[0]:''" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../lvyoujingdian/detail.jsp?id='+item.id">查看详情</a>
          ...
          <div class="caseBox_2">
            <a :href="'../lvyoujingdian/detail.jsp?id='+item.id">{{item.jingdian}}<span style="color: red;float: right;" v-if="item.price">{{item.price}} RMB</span></a>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery'], function() {
      ...
      http.request(`lvyoujingdian/autoSort`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
      ...
      http.request(`lvyoujingdian/list`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
        vue.lvyoujingdianList = res.data.list;
      });
    });
  </script>
</body>
