      ...
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="jingdian" id="jingdian" placeholder="景点" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="jijie" id="jijie" placeholder="季节" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
      ...
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        <div class="proBoximg">
          <div class="primg" style="background-size: cover;background-repeat: no-repeat;background-position: center;">
            <img :src="item.fengmian?item.fengmian.split(',')[0]:''" width="390" height="320" />
          </div>
        </div>
        <div class="proBoxtxt">
          <p>{{item.jingdian}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#jingdian').val()) {
          param['jingdian'] = jquery('#jingdian').val() ? '%' + jquery('#jingdian').val() + '%' : '';
        }  
        if (jquery('#dengji').val()) {
          param['dengji'] = jquery('#dengji').val() ? '%' + jquery('#dengji').val() + '%' : '';
        }  
        if (jquery('#jijie').val()) {
          param['jijie'] = jquery('#jijie').val() ? '%' + jquery('#jijie').val() + '%' : '';
        }  
